...
| .worktrees/chore/keyframes-full-version-name | chore/keyframes-full-version-name | Claude Fable 5.1 (orchestrator + subagents) | 1 (follow-up) | DT-047 + DT-048(a): keyframes named by full catalog version, replace same-major freeze gate with id/param-key superset gate, Kotlin `keyframesName()` + name-agreement test, docs | in progress | 2026-09-18 | yes — behaviour of `keyframesName()`, `schema.json` description text only |
...
- 2026-09-18 — **Decision from Chris (DT-047): keyframes are named by the full catalog version**, `vm-<id>-v<M>-<m>-<p>` (e.g. `vm-fade-in-up-v1-1-0`), not `-v<major>`. Semver clarified: a new knob whose default reproduces the old rendering is a MINOR; MAJOR = changed rendering at default params, removed ids, renamed/removed params. In flight on `chore/keyframes-full-version-name`. Always call `keyframesName()`; never build or parse the name yourself. Phase 3: after it merges, rebase; any test expecting 1.0.0 and 1.1.0 to share one `@keyframes` block will no longer hold. Phase 4: the bridge must get the name from the shell/shared generator. Phase 7: do not write goldens until this merges.
...
| DT-047 keyframes full-version naming | Claude Fable 5.1 (session w/ Chris) · worktree `.worktrees/chore/keyframes-full-version-name` | 2026-09-18 |
