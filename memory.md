...
| .worktrees/feat/phase-1-catalog | feat/phase-1-catalog | Claude Fable 5.1 (orchestrator + subagents) | 1 | Close Phase 1 exit criteria: cross-language (version, animationId) agreement, catalog 1.1.0 adding `fillMode` standard param | PR #3 ready for review (gates green on 901add9, reviewer approved) — awaiting merge by Chris | 2026-09-18 | yes — `schema.json` (additive: `fillMode` standard key), new `versions/1.1.0.json`, `current` bump |

## Notices (newest first)

- 2026-09-18 — **Phase 1 PR #3 ready for review.** Contract facts for Phases 2/3/4: catalog `1.1.0` is `current`; `fillMode` is the sixth standard param key (`select`, options none/forwards/backwards/both) on every entry from 1.1.0; `validate` now enforces that versions sharing a MAJOR keep identical keyframes/baseStyles per animationId (DT-047) and that `select` params have `default ∈ options`. `packages/animation-catalog/manifest.json` is generated (`pnpm gen:types`) and gate-checked. `package.json` entry-point block overlaps with `feat/3-web-shell-help`: second merger rebases, keep `dist/src/`.
