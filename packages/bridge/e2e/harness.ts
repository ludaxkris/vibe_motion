...
const EXPORT_SCRIPT_SOURCE = readFileSync(path.join(here, "..", "src", "vibe-motion-export.js"), "utf8");
...
/** A designer's own site, serving what the Phase 7 exporter produced. No app, no api, no bridge. */
export const EXPORT_ORIGIN = "http://exported.test";
/** A third site that frames the exported page, so the observer's `rootBounds` comes back null. */
export const EMBEDDER_ORIGIN = "http://embedder.test";
...
// ---------------------------------------------------------------------------
// The exported page (Phase 7).
//
// `src/vibe-motion-export.js` is a constant file the exporter returns unchanged, so it is exercised
// the same way the bridge is: served by `page.route` on an origin of its own, with no app, no api
// and no bridge anywhere near it. The stylesheet below is hand-written to the exact shape
// `CssEmitter` produces; the full-stack spec in `apps/e2e/web/stack` is what proves the real
// generated bytes agree.
// ---------------------------------------------------------------------------

export type ExportedAnimation = { name: string; time: number; state: string } | null;

export type ExportHarness = {
  page: Page;
  /** The document the exported page lives in: the page itself, or the frame it was embedded in. */
  frame: Frame;
  /** The element's first animation, as plain data, or null when it has none. */
  animation(selector: string): Promise<ExportedAnimation>;
...
export function exportedCss(classes: string[]): string {
  const rules = classes
    .map(
      (name) =>
        `  :where(.vm-js) .${name} {\n` +
        `    --vm-distance: 24px;\n` +
        `    animation: vm-fade-in-up-v1-1-0 5000ms linear 0ms 1 normal both;\n` +
        `  }`,
    )
    .join("\n\n");

  return (
    `@keyframes vm-fade-in-up-v1-1-0 { from { opacity: 0; transform: translateY(var(--vm-distance)); }` +
...
/**
 * Serve an exported page and open it.
 *
...
export async function mountExport(
  ...
  // Installed before any page script, so it records the very first frame — which is before the
  // first paint, and therefore proves the non-deferred script ran in time.
  await page.addInitScript(() => {
    window.__firstFrameClass = null;
    window.__starts = {};
  ...
  await page.goto(options.embed ? `${EMBEDDER_ORIGIN}/` : `${EXPORT_ORIGIN}/index.html`);

  const frame = options.embed
    ? page.frames().find((candidate) => candidate.url().startsWith(EXPORT_ORIGIN))
    : page.mainFrame();
  if (!frame) throw new Error("the exported page never loaded");
...
declare global {
  interface Window {
    ...
    /** `<html>`'s class list at the first animation frame, i.e. before the first paint. */
    __firstFrameClass: string | null;
  }
}
