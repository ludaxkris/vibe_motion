...
// export-parity.json is generated by apps/web's own vitest run from the real `lib/runtime-css`
// functions and committed there. ExportParityTest reads it to prove the Kotlin CSS emitter and the
// TypeScript preview generator agree for every published (catalogVersion, animationId) pair. Test
// resources only, like the manifest above: the Docker build runs no tests, so the production image
// gains no dependency on apps/web.
val exportParityDir = layout.buildDirectory.dir("generated/exportParity")

val exportParityResources =
    tasks.register<Sync>("exportParityResources") {
        description = "Copies apps/web/lib/runtime-css/export-parity.json into API test resources only."
        group = "build"
        into(exportParityDir.map { it.dir("export") })
        from(layout.projectDirectory.file("../web/lib/runtime-css/export-parity.json"))
    }

sourceSets {
    test {
        resources.srcDir(files(catalogManifestDir).builtBy(catalogManifestResources))
        resources.srcDir(files(exportParityDir).builtBy(exportParityResources))
    }
}
