...
import { MAX_LABEL_LENGTH, summariseDiff } from "@/lib/diff-summary";
...
describe("while viewing, the store is read-only", () => {
  ...
  it("is not 'unsaved' even with Phase 5 provenance in the state", () => {
    const store = viewingStore();
    // `applyDraftReplacement` clears `generated` on the way in, and every
    // writer that could put it back is a no-op while viewing, so this state is
    // unreachable through the actions — which is the point: `selectUnsaved`
    // must answer on `mode` alone, not on whether a run happens to be
    // remembered. `draftState` is a viewer buffer here, not a draft.
    store.setState({ generated: { "vm-1": assignmentFor("fade-in") } });

    expect(selectDirtyVmIds(store.getState())).toEqual([]);
    expect(selectUnsaved(store.getState())).toBe(false);
    expect(selectDirtyVmIdCount(store.getState())).toBe(0);
  });
});

/**
 * The one flow that crosses both phases end to end: the page auto-generate
 * fills the draft, and Save has to treat what it wrote as ordinary unsaved
 * work — `generated` is client-only and never reaches a diff (plan D2).
 */
describe("Save after a page auto-generate", () => {
  /** An 11-element run, the shape a real page auto-generate produces. */
  function autoGeneratedPage() {
    const store = createEditorStore();
    store.getState().loadVersion("version-1", {});
    const assignments: EditorStateMap = {};
    for (let i = 0; i < 11; i += 1) assignments[`vm-${i}`] = assignmentFor("fade-in-up");
    store.getState().applyPageSuggestion({
      suggestion: { assignments, skipped: [] },
      seed: 1,
      prompt: "",
      truncated: false,
  ...
  it("posts every agent-made assignment in the diff", () => {
    const store = autoGeneratedPage();
    const state = store.getState();

    const diff = computeDiff(state.currentVersionState, state.draftState);

    expect(Object.keys(diff.set)).toHaveLength(11);
    expect(diff.remove).toEqual([]);
    expect(selectAgentOwnedVmIds(state)).toHaveLength(11);
  });

  it("prefills a label the contract's 200-character cap accepts", () => {
    const store = autoGeneratedPage();
    const state = store.getState();

    const { label } = summariseDiff(
    ...
    expect(label).toContain("+8 more");
  });

  it("markSaved(version, posted) clears unsaved and keeps the work agent-owned", () => {
    const store = autoGeneratedPage();
    const posted = store.getState().draftState;
    const generated = store.getState().generated;
    const lastRun = store.getState().lastRun;

    store.getState().markSaved(version({ id: "version-2" }), posted);

    const after = store.getState();
    expect(selectUnsaved(after)).toBe(false);
    expect(after.currentVersionId).toBe("version-2");
    // Untouched agent work stays the agent's after a Save, so Regenerate may
    // still re-roll it — as a new unsaved change (plan D2).
    expect(after.generated).toBe(generated);
    expect(after.lastRun).toBe(lastRun);
    expect(selectAgentOwnedVmIds(after)).toHaveLength(11);
  });
});
