...
| e2e | `apps/web/e2e` | Playwright | Fixture pages under `apps/web/e2e/fixtures`. Specs that need the real api live in `apps/web/e2e/stack/` and run only in the per-run Docker stack (`pnpm e2e:docker`, see `docker/e2e/README.md`); import origins from `stack/env.ts`. Cover: clone → select → Generate; Custom → pick → tune; Auto-generate; version view/restore; export runs in a fresh page. |
