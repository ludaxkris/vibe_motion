...
- **Which version the tab exports (Track C).** One explicit version, not "whatever is on screen": the project's current version by default, and the version being viewed when the tab is opened from History or from a row's **Export vN** (`docs/plans/phase-7-export-plan.md` §5.2). Leaving the History tab still returns the canvas to the current version, so the panel's own `v3` label is what says which version is being exported. Opening Export with unsaved changes runs the existing tab guard: **Save first** lands on the version that save created, **Discard** on the current version, and a cancelled save leaves the reader on their draft. No export is ever requested for an unsaved draft.

What shipped (Phase 7, 2026-09-20)

- **Track A** — the Kotlin exporter: a base-free `HtmlSanitiser` extracted from the clone pipeline (DT-073), `CssEmitter` / `HtmlEmitter` / `InViewScript`, `GET /projects/{id}/export` with `Cache-Control: no-store` and the error codes in plan §1.6 (400 `missing_vm_id`, 404 for a snippet of an unassigned element), goldens for three fixture states, and a generated parity fixture that keeps the Kotlin emitter and `apps/web/lib/runtime-css` in agreement for every published `(catalogVersion, animationId)`.
- **Track B** — the Export tab's components: file tabs, the ink code block (bundle text rendered as text, never as HTML), client-side zip via `fflate`, clipboard with a fallback, and `/dev/export`.
- **Track C** — the wiring: the tab mounted on the version above, "Export vN" from a History row (DT-160), save-first through Phase 6's `requestSave()` (DT-099's Export half), the MSW mock aligned to the real file names and 404 rule, and the export script hardened against a host re-render that rewrites `class` (DT-187).

Exit criteria: golden-file tests for three fixture states; Playwright opens the exported HTML in a fresh page and asserts the animation runs. **Met**, by `apps/api/src/test/kotlin/dev/vibemotion/api/export/ExportGoldenTest.kt` and `apps/e2e/web/stack/export.spec.ts` (the three files served from a foreign origin with no app, no bridge and no CSP of ours: load runs, in-view is held at `currentTime` 0 and plays once when reached, hover animates only while hovered, reduced motion and JavaScript-off leave the page readable). `apps/e2e/web/stack/export-ui.spec.ts` adds the pass through the tab itself — animate, Save, Export, and a downloaded zip whose `index.html` and `vibe-motion.css` are the bytes the API built.
