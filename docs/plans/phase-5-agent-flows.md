...
export type SkipReason = "not-semantic" | "too-small" | "hidden" | "nested"; // "nested": inside a container target (D6)
...
`autoGeneratePage`: `seed = regenerate && lastRun ? lastRun.seed + 1 : now()`; `queryElements({ filter: { tags: TARGET_TAGS, minWidth: MIN_TARGET_WIDTH, minHeight: MIN_TARGET_HEIGHT }, limit: 200 })`; on Regenerate, for vmIds already in `store.elements` reuse the remembered `ElementInfo` (a fresh measurement of a mid-animation element returns its transformed box and can flip `load`/`in-view` or `too-small`); `selectAutoCandidates`; a throwing agent → `agent-failed`, nothing written; `suggestForPage`; zero assignments → `no-targets` and nothing is written; else `applyPageSuggestion`. `generateForElement`: uses `store.elements[vmId]` (always present: selection stores it), viewport from `lastRun.viewport` if any, else `{ width: element.rect.width + element.rect.x, height: Number.MAX_SAFE_INTEGER }` so an unknown fold means `load`; then `applyGenerated`.
