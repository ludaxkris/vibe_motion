...
12. **Fixture:** there is NO `<li>` (the earlier count matched `<link`). Real census: 1 `h1`, 3 `h2`, 4 `p`, 3 `article`, 1 `img`, 1 `a`. Nothing sits below the fold, and the fold is the preview iframe's `innerHeight` (~600 px), not 800. Add a `<section class="spacer">` with `min-height: 1600px` followed by one `h2`, and raise `.cta` padding to `16px 24px` (it is ~42 px tall against `MIN_TARGET_SIZE = 40`). Check `stack/bridge.spec.ts` and `stack/editor.spec.ts` still pass with the taller page.
