...
| D2 | **(owner) Auto-generate keeps the user's work and re-rolls the agent's.** An element is *agent-owned* iff `assignmentsEqual(draftState[vmId], generated[vmId])`, where `generated` is a client-only map of each assignment exactly as the agent produced it. Any hand edit (param, trigger, re-pick) breaks the equality, so ownership lapses with no bookkeeping on edit paths. Auto-generate and Regenerate assign only to elements with no draft assignment or an agent-owned one; everything else goes to the agent as context. "Remove all" removes agent-owned assignments only. **Guard:** Phase 4's element-switch guard treats an element as dirty only when it differs from `currentVersionState` **and is not agent-owned**, so an untouched generated element never opens "Save changes to …?"; a hand-tuned one does, and Discard restores `generated[vmId]` when `currentVersionState[vmId]` is absent (else Phase 4's behaviour). "‹" back to the result list deliberately does not guard. **Stale provenance:** `PICK` (when it creates an assignment) and `removeDraftAssignment` delete `generated[vmId]`, so a later hand pick that happens to equal an old suggestion is not re-rolled. **Async re-check (PR #21 review):** `applyGenerated` re-checks ownership when the agent's answer lands, like `applyPageSuggestion`: an element the designer animated meanwhile is left alone, and the panel moves to `tuning` only if it is still `selected` on that element (never out of the picker). **Phase 6 hand-off:** `applyDraftReplacement` (Phase 6's funnel for `loadVersion` / `enterViewing` / `exitViewing` / `rebaseDraft`) clears `generated` (keeping identity when already empty) and sets `lastRun: null`; `markSaved` must NOT clear them (saved, untouched agent work stays re-rollable, and Regenerate re-rolls it as a new unsaved change); `reset` and `revertDraft` already clear both. `applyGenerated`, `applyPageSuggestion`, `removeAllGenerated` and `run.ts` return early while `mode === "viewing"`: the three actions are identity no-ops, and both `run.ts` entry points return `{ ok: false, reason: "query-failed" }` before querying (implemented in this phase; the `mode` field already exists). | One click must never discard hand tuning. Deriving ownership from equality keeps edit paths free of bookkeeping; the one Phase 4 seam that changes is `selectElementDirty` (Task 4). |
...
| D6 | **Heuristics.** Targets: visible, border box ≥ 40 px wide and ≥ 16 px tall (`MIN_TARGET_WIDTH` / `MIN_TARGET_HEIGHT`; originally 40×40, changed when the stack e2e found that browser-default headings, `h1` ≈ 37 px and `h2` ≈ 28 px, and one-line paragraphs, ≈ 18 px, are under 40 px tall: one line of text must pass, icons, hairlines and 1-px spacers must not), tag in `h1–h6, p, li, blockquote, img, video, figure, article, button, a` (`picture` was dropped in PR #15: it is an inline box, so its own transform is a no-op, and `picture > img` double-animates) or `role="button"`; **a block animates as one unit**: an entrance target whose `pageRect` lies fully inside (0.5 px tolerance) the `pageRect` of a larger target with a container tag (`CONTAINER_TAGS = article, figure, li, blockquote`) is skipped as `nested`, so the outermost container stays and a card no longer gets three entrances with its `fillMode: both` holding `opacity: 0` over its children's (DT-133); this matches the handoff mock's `.plan ×3` row. Hover targets (`a`, `button`, `role=button`) are never nested: a link inside an animated card keeps its hover. **Nesting is decided against every listed element that holds an entrance, user-owned included** (PR #21 review): `run.ts` passes the listed elements that are not candidates and whose existing assignment has a trigger other than `hover` as `PageContext.context` (additive, optional), and `selectTargets(elements, { blocks, viewport })` counts those with a container tag as containers without ever reporting them as targets or skipped, so a card the designer animated by hand still shields its heading and text on the next run. **Too large:** with a `viewport`, an entrance element whose `pageRect.height > viewport.height` is neither a target nor a block (`SkipReason` `too-large`): it is excluded from the containers, so what is inside it is judged on its own. A wrapper `article` around a whole post would otherwise be the page's only row, and below the fold an `in-view` entrance on something taller than 5 × the viewport can never reach the bridge's 0.2 visibility threshold, leaving it held at `opacity: 0`. "Taller than one screen" needs no tuning constant. Without a `viewport` the rule is off; `suggestForElement` ignores it (the user asked for that element). `button`, `a`, `[role=button]` → category `hover`; everything else → category `entrance`. Never `exit`. `EXCLUDED_ANIMATION_IDS = ["underline-sweep", "shimmer"]` (paint at rest, DT-116 class) is a guard only: both are outside the `entrance`/`hover` pools today, so its test asserts the filter on a synthetic pool, not on the live catalog. Trigger: hover picks use `hover`; entrance picks use `load` when `pageRect.y < viewport.height`, else `in-view`. Stagger: the n-th `load` entrance in document order gets `delay = min(n × 60, 600) ms`; `in-view` and hover get the catalog default. All other params are catalog defaults. | build_plan Phase 5 (60 ms wins over the mock's 120 ms: docs win on behaviour). Values stay inside catalog min/max because the API validates them on Save (422 `invalid_diff`). |
| D7 | **Determinism.** `MockAnimationAgent` takes a numeric seed; unit tests pin seeds. The agent reseeds on **every call** (PR #15), so one instance is deterministic however often it is called, and a repeated per-element generate must pass a new seed to get a different pick. The app seeds from `Date.now()`; Regenerate uses `lastRun.seed + 1`. e2e asserts seed-independent invariants (categories, triggers, stagger, no exit), not exact picks. | No test-only seed channel into a production build. |
...
| A3 | `feat/5-panel-auto-state` | 3 | now (**merged 2026-09-19 as PR #17**, ahead of Phase 4) | `panel-machine.ts` + one line in `store/index.ts` (`selectSelectedVmId`; `selectGuardedVmId` already reads through it) |
...
export type SkipReason = "not-semantic" | "too-small" | "hidden" | "nested" | "too-large"; // "nested": inside a container (D6); "too-large": entrance taller than the viewport (D6)
...
// Not square: one line of text must pass. Found by the stack e2e: browser-default headings and
// one-line paragraphs are under 40 px tall.
...
export function skipReason(el: ElementInfo): SkipReason | null {
  ...
  if (el.pageRect.width < MIN_TARGET_WIDTH || el.pageRect.height < MIN_TARGET_HEIGHT) return "too-small";
  return null;
}
...
- `CHOOSE_CUSTOM`, `PICK`, `CLEAR` preserve `returnTo`. **`REVERT` drops it** (decided in the PR #17 review): reverting throws away everything the agent generated, so there is no list to go back to; the target state is otherwise unchanged.
...
- [ ] Tasks 1–3 are on `main`; `ready.bridgeVersion` is `1.1.0` in the stack image. (2026-09-19: #15 and #17 are merged; #17 landed **before** Phase 4, so Phase 4's PR #18 resolves the two control-panel conflicts, not this track. #16 is still open.)
- [ ] Left for Track B by PR #17, all in `control-panel/index.tsx`'s `auto` case: rows are empty, `regenerateDisabled` / `replayDisabled` / `removeAllDisabled` are set, and `consideredLimit={0}` is a placeholder. Task 5 wires them and passes `ELEMENTS_QUERY_LIMIT` from `bridge`.
- [ ] DT-134: with the bridge pre-filter, `skippedCount` is always 0, so the "Skipped n elements" caption never renders. Decide in Task 5: leave it dormant (default) or have `elements:list` report a filtered-out count (additive).
- [ ] Bridge 1.1.0 withholds `ack` for a non-finite envelope `seq` on every message type; the client's `seq` counter must stay a finite integer (it is, per Phase 4 Task 7).

#### Task 0 result (2026-09-19, against `main` at PR #18's merge) — read this before Tasks 4–6; where it differs from the task text below, THIS wins

Holds: `createEditorStore()` / `EditorStoreApi`; `selectElementDirty` is the single dirty decision inside `requestSelect`; `resolveGuard("discard")` restores one key (`guardedVmId`) from `currentVersionState`; the client's `seq` is a monotonic integer; a new `ready` rejects every pending promise (`"preview frame reloaded"`); the subscriber coalesces per rAF flush and sends one `state:load` above `BULK_APPLY_LIMIT`; `whenIdle()` drains `pending`; nothing reads `panel.vmId` behind a bare `status !== "idle"`; PR #17's `auto` branch survived; `transpilePackages: ["bridge"]` is set, so VALUE imports from `bridge` work.

Wrong, with the real fact:
1. **Bridge is `1.1.1`**, not 1.1.0 (PR #18 bumped it). Numeric compare `>= 1.1.0`; put the compare in `lib/bridge/semver.ts` (`atLeast(version, "1.1.0")`, non-numeric or missing → false) with its own test.
2. **The client discards `ready.bridgeVersion`.** `onReady` must store it (reset on every `ready`).
3. **`queryElements` cannot reuse `pending`** (`Map<number, Deferred<Ack>>`, and `onAck` resolves with the `Ack`, even when `ok:false`). Add a second registry `queries: Map<number, { resolve, reject, timer }>` and `case "elements:list"` in the inbound switch (today it hits `default`). Send through the normal `send()` so the seq is ALSO in `pending` (that is what keeps `whenIdle()` honest). Resolve the query on the matching `elements:list`; reject it on its ack when `ok === false` (`Error("elements-query-rejected")`), after 3000 ms (`Error("elements-query-timeout")`, its own timer, distinct from the 5 s ack timeout), on `settleAll` (new `ready`, `destroy`), and up front when not ready or `!atLeast(bridgeVersion, "1.1.0")` (`Error("bridge-too-old")`). Validate every element with the existing `asElementInfo`; drop invalid ones.
4. **The panel cannot reach the client**: `ControlPanel` gets three callback props, no context. Decision: stay with props. `editor-shell.tsx` builds them from `lib/agent/run.ts` with `{ store, bridge: client }` and passes, only while `bridgeReady`: `onReplay: (vmId: string | null) => void` (widened), `onGenerateElement: (vmId: string) => Promise<RunOutcome>`, `onAutoGeneratePage: (opts?: { regenerate?: boolean }) => Promise<RunOutcome>`. Undefined ⇒ the buttons render disabled. `IdlePanel` / `SelectedPanel` stay store-free and take plain props (`prompt`, `onPromptChange`, `onAutoGenerate`, `busy`, `error`; `onGenerate`, `busy`, `error`).
5. **Busy / error state lives in `ControlPanel`** (a small `useAgentRun()` hook: one in-flight run at a time, so a double click runs once; last failure reason → the D10 copy; cleared on the next run). Not in the store.
6. **`applyPageSuggestion` must call `transition(state.panel, { type: "AUTO_DONE" })` inside its own updater** (the `revertDraft` pattern); `dispatchPanel` is a separate `set()`.
7. **New store fields go into `initialEditorState`** (`reset()` is a shallow merge of it). `selectAgentOwnedVmIds` must NOT use the module-scope `dirtyCache` (keyed on `draftState` + `currentVersionState` only).
8. **Only the element-switch guard ignores agent-owned work.** `selectUnsaved`, `selectDirtyVmIdCount`, `selectGuardedVmId` and the tab-switch guard keep counting generated assignments: they ARE unsaved, the indicator must show, and leaving for History/Export should still ask.
9. **Row click uses `requestSelect`** (today `setSelectedVmId`). From `auto` nothing is selected, so it never opens the guard; from a tuned element it correctly can.
10. **`IdlePanel` has its own disabled "Replay all"** (idle.tsx ~123, asserted at `idle.test.tsx` ~92): wire it to `onReplay(null)` too. Also pass `elementText={elements[vmId]?.textPreview}` to `SelectedPanel` (built, never wired).
11. **e2e:** no postMessage recorder and no click helper exist to reuse; `window.__vmTest` does not exist in the production stack build. Task 6 writes its own `page.addInitScript` recorder (frame side, `source === "vibe-motion"`, push `data.type`), and lifts `preview(page)` / `cloneFixture(page)` / an `inline(locator, prop)` reader into `apps/e2e/web/stack/helpers.ts` (move, do not copy; `bridge.spec.ts` imports them). `UnsavedIndicator` gets `data-testid="unsaved-indicator"`.
12. **Fixture:** there is NO `<li>` (the earlier count matched `<link`). Real census: 1 `h1`, 3 `h2`, 4 `p`, 3 `article`, 1 `img`, 1 `a`. Nothing sits below the fold, and the fold is the preview iframe's `innerHeight` (~600 px), not 800. Add a `<section class="spacer">` with `min-height: 1600px` followed by one `h2`, and raise `.cta` padding to `16px 24px` (it was ~42 px tall against the then 40×40 floor). **The floor is now 40 px wide × 16 px tall** (`MIN_TARGET_WIDTH` / `MIN_TARGET_HEIGHT`): the stack e2e found that browser-default headings and one-line paragraphs are under 40 px tall, so the old floor skipped exactly what Auto-generate exists for; the fixture keeps default-sized headings and the spec asserts the `h1` is < 40 px tall and still gets a row. Check `stack/bridge.spec.ts` and `stack/editor.spec.ts` still pass with the taller page.
13. DT-134 is no longer dormant: `skippedCount` is whatever `selectTargets` reports, and that is now real, because nested elements pass the bridge pre-filter and are skipped by the agent (9 on the e2e fixture), so the "Skipped n elements (inside an animated block, too large, too small, hidden or not content)." caption is live. Still no protocol change.
...
`autoGeneratePage`: `seed = regenerate && lastRun ? lastRun.seed + 1 : now()`; `queryElements({ filter: { tags: TARGET_TAGS, minWidth: MIN_TARGET_WIDTH, minHeight: MIN_TARGET_HEIGHT }, limit: 200 })`; for a vmId that has a draft assignment when the run starts, reuse the `ElementInfo` remembered in `store.elements` (a fresh measurement of a mid-animation or held element returns its transformed box and can flip `load`/`in-view`, `too-small` or nesting), unless the previous run's viewport differs from this list's, in which case the layout has changed and the fresh list wins; the resting boxes are then written back with `rememberElements`, because the client has just overwritten them with the measured ones (PR #21 review; originally gated on Regenerate only, which missed a second run from idle); `selectAutoCandidates`; a throwing agent → `agent-failed`, nothing written; `suggestForPage`; zero assignments → `no-targets` and nothing is written; else `applyPageSuggestion`. `generateForElement`: uses `store.elements[vmId]` (always present: selection stores it), viewport from `lastRun.viewport` if any, else `{ width: element.rect.width + element.rect.x, height: Number.MAX_SAFE_INTEGER }` so an unknown fold means `load`; then `applyGenerated`.
