...
| `in-view` | one shared `IntersectionObserver` for all in-view assignments | thresholds `[0, T]`, `T = IN_VIEW_THRESHOLD = 0.2`, the exported constant in `protocol.ts` that the Phase 7 exporter reuses. The `0` is what makes the reachability clause of §6a possible: an element that can never reach `T` has to be reported the moment it intersects at all. Measured in Chromium, it costs **two** more callbacks per element per scroll-through — crossing `0` on the way in and on the way out, not one per entry — and for an element that can reach `T` both leave through the `record.armed === next` early-out before any style write or flush. No per-frame work and no timer either way |
...
| `in-view` threshold | `IN_VIEW_THRESHOLD`, as observer thresholds `[0, T]` | same constant, same two thresholds |
| `in-view` firing condition | `isIntersecting && (intersectionRatio >= T \|\| reachable <= T)`, where `reachable = min(1, rootW/w) * min(1, rootH/h)` — the largest ratio the element could ever attain, given how much of it fits in the root on each axis — and `rootW`/`rootH` come from `entry.rootBounds`, falling back to `window.innerWidth`/`innerHeight` when it is null. The second clause is DT-095: `intersectionRatio` is an **area** ratio, so an element bigger than the root can never reach 1 and one big enough can never reach `T` at all, which would hold it on its first keyframe for the whole session. Area, not height: a 4000px-wide track in a horizontal scroller reaches 0.16 with nothing wrong with its height, and a 1280×1200 element is short on neither axis alone. `<=`, not `<`: an element whose best possible ratio is exactly `T` could only reach it perfectly aligned, which is float rounding deciding whether an animation ever plays. The observer's `0` threshold is what makes the clause possible at all — without it the browser reports nothing until `T`, which such an element never reaches. `rootBounds` is null for an implicit root inside a cross-origin iframe, which is **every** bridge there is, so the fallback is the preview's normal path rather than an edge case, and reading off null would throw inside the callback and hold everything for ever. A **zero-size root** is not an unreachable one: a frame collapsed to no height still reports anything touching its edge as intersecting at ratio 0, and `min(1, 0/h) = 0` would read as unreachable and play the element where nobody can see it, leaving it armed so it never plays when the frame comes back. Both sizes are guarded and yield `1`, which holds the element until there is something to be in view of. Still uncovered, and logged as DT-184 for both sides: a clip container narrower than the root, which bounds the intersection without appearing in `rootBounds` | the same condition, in `src/vibe-motion-export.js` (plan task A12 made the two agree; separate copies because neither file can import the other, so the rule is stated in both and this row is the contract between them). The zero-root guard lands there in the Phase 7 Track C PR, which owns that file and its golden. In an export the null `rootBounds` really is the edge case: an exported page embedded in a cross-origin iframe |
...
| `in-view` repeats | re-arms on every entry, so the designer can see it again by scrolling (deliberate divergence). Disarming is the firing condition's negation, so an element that can reach `T` is held again below it and one that cannot is held again only when it stops intersecting at all | plays once: the element is unobserved when it fires, so nothing disarms it |
