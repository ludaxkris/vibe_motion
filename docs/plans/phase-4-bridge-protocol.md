...
- **Rects are the element's RESTING box, never the painted one.** `rect`, `pageRect`, the `minWidth` / `minHeight` floor and `visible` are all judged on it, and the bridge — not the shell — is the single source of truth for it. Two cases, decided per element:
  - **Nothing of ours applied** to the element or to any ancestor of it: exactly `getBoundingClientRect()`, as before. A page with no assignments measures byte for byte as it did.
  - **Something of ours applied** to the element or to any ancestor — applied or merely previewing, any trigger, any play state, running, paused or finished: the **layout box** (`offsetLeft`/`offsetTop`/`offsetWidth`/`offsetHeight` accumulated up the `offsetParent` chain, which no `transform` can reach). A CSS transform applies to the whole subtree, so an ancestor's held first keyframe is what would otherwise distort a child that has no assignment at all.
  This exists because an `in-view` element is held on its first keyframe for as long as it is off screen (D3): measured live, a slide-in is displaced by its distance, a scale-from-zero shrinks, and `flip-in-x`'s `rotateX(90deg)` measures **zero height** — under the floor, `visible: false`, out of the list entirely, and its children stop being geometrically inside the block they belong to. A measurement that depends on which frame of an animation a query landed in cannot be the basis of a size filter or of a nesting test.
  The layout box's limits, for a touched subtree only: it is **integer-snapped** (`offsetWidth` is an integer, so edges can differ from the live rect by up to 0.75 px — consumers comparing two rects need a tolerance to match, see `apps/web/lib/agent/targets.ts`); it is **blind to a host ancestor's own transform** (a page that scales a wrapper reports the un-scaled size for the touched element, DT-198); and it reports the **first fragment of a wrapped inline** rather than the union. An element with **no offset box** (`<svg>`, MathML) is not an `HTMLElement`, so it is measured live whatever is applied, and can still measure zero and drop out while held (DT-200).
  The same rule covers `elements:list`, `element:select` and `element:hover` alike: all three build their payload with one `elementInfoFor`. Lands as bridge **1.1.2 or 1.1.3** — whichever of PR #27 (in-view reachability) and PR #28 (this rule) merges second carries the bump, and its README changelog line covers both behaviours.
...
| Hover outline, selection ring and its label | one fixed-position container `<div data-vm-overlay>` appended to `<body>`, `pointer-events: none` | hover / `select`; repositioned in `requestAnimationFrame` from a capture-phase passive `scroll` listener (nested scrollers do not bubble), from `resize`, and from a `ResizeObserver` on `<html>` and on the two boxed elements |
...
**The ring marks the element's *resting* box — the box it occupies with our animation's transform taken off — and it keeps marking it at all times, following every scroll, resize and reflow, including while one of our animations is running and including an `iteration: infinite` one that never fires `animationend`.** While our animation is running the box is derived from `offsetLeft`/`offsetTop`/`offsetWidth`/`offsetHeight` accumulated up the `offsetParent` chain, which no transform can reach, corrected by the difference between that and `getBoundingClientRect()` recorded the last time the element was measured at rest; at rest the box is `getBoundingClientRect()` exactly. A host page's own animation on the element is not ours and the ring follows it live. There is no per-frame loop and no timer. The one exception is an element with no offset box (`<svg>`, MathML): it is not an `HTMLElement`, so its ring is always the live bounding box; it still follows every scroll, resize and reflow and is never empty.
...
- **Outbound:** a store subscription diffs `draftState` by reference per vmId. Changes are coalesced per vmId per `requestAnimationFrame`: `apply` for changed entries, `clear` for removed ones. More than `BULK_APPLY_LIMIT` (8) changed entries in one update sends a single `state:load` instead (this is what Phase 5 auto-generate produces). **`state:load` re-seeds the whole page, so it also restarts the `load` animations of elements that did not change in that update** — one message instead of dozens, at the cost of a noisier replay. That is acceptable for its only caller, auto-generate, where the designer has just asked for the whole page to change anyway; anything that must not disturb unrelated elements has to stay under the limit and send per-vmId `apply`s. When a slider is released (pointer up) the shell sends `replay` so the designer sees the result. Selection changes send `select`. Nothing here calls the API.
- The iframe gets `sandbox="allow-scripts allow-same-origin"`. `allow-same-origin` keeps the framed document on its own origin so the origin check and `script-src 'self'` work; dropping it would make the frame's origin `"null"` and force a `"*"` target. The pair is safe only while the framed page is cross-origin with the shell, so the shell **asserts** that rather than assuming it: outside mock mode a preview origin equal to `location.origin` renders an error in place of the iframe, and inside mock mode (where the framed document is the repo's own fixture) it warns.
- **Client rules.** Four things the client owes the frame, each of which used to be a way for the channel to get stuck or to lie:
  - Nothing but `hello` is posted before `ready`. A message posted to a frame whose document is not on `expectedOrigin` yet is dropped by the browser and never acked; queuing is pointless because the whole state is re-derived and sent as one `state:load` at `ready`. `hello` itself is not tracked as a pending ack — spec D7 already says its ack proves nothing.
  - Every tracked `seq` carries a deadline (5 s) and rejects when it expires, so a stuck frame cannot make `whenIdle()` hang or `pending` grow without bound.
  - `replay` flushes the coalescing frame before it is posted. `postMessage` to one window is ordered, so that is enough to guarantee an `apply` from the same turn arrives first; no ack round trip is needed.
  - A preview's lifetime is owned by the component that started it (the animation card, which ends it on unmount as well as on pointer-out), and the client clears an active preview before any `apply`, `clear` or `state:load` that would land under it. A preview sits on top of the applied assignment (D6), so anything written underneath is invisible while it is up.
- An ack that comes back `ok: false`, or one naming `unknownVmIds`, is surfaced (`onAckError`): the store and the frame now differ and only the panel can say so. Inbound `ElementInfo` is shape-checked before it is stored.

## 6. Performance budget

| Path | Budget | How it is met / measured |
|---|---|---|
| Param change (slider tick) | shell-side round trip (store set → `ack` received) p95 < 16 ms; `ack.ms` p95 < 4 ms | inline `setProperty` calls only, no stylesheet write, no layout read. Measured in Playwright with 120 rAF-paced `updateDraftParam` ticks under CDP 4× CPU throttling. p95, not max, because CI is noisy. |
| `state:load`, 200 assignments | `ack.ms` < 50 ms | one stylesheet text write; style writes batched; no per-assignment layout read |
| `elements:query`, **tag-filtered** (the Phase 5 filter: `h1`–`h6`, `p`, `li`, `blockquote`, `img`, `picture`, `video`, `figure`, `article`, `button`, `a`; min 40×40), 2,000-element page | p95 `ack.ms` < 50 ms with guaranteed-dirty layout, zero style writes | tag/role pre-filter before any measurement; one read-only loop. Measured in `packages/bridge/e2e` on a generated page (2,121 tagged elements, the ~300 KB of text inside nested `<article>` wrappers, which the filter matches) at `limit` 200, at the 500 ceiling, and with sizes nothing reaches (full scan), 40 runs each clean and 40 with the whole page's layout invalidated in the same task as the query, so no frame can flush it first. p95, not max, because CI is noisy. A `MutationObserver` asserts no write. An untagged query is outside this budget (§3). Also with **every candidate under one applied assignment**, which is the ordinary state after an Auto-generate run and takes the resting-box path for every element: roughly 2× per candidate, and over budget on a full scan at 4× today (DT-199). |
...
**Where the budgets are defined, and what gates them** (amended 2026-09-19, after CI measured 97–172 ms for the round trip and 4.3 ms for `ack.ms` on a shared runner):

- The millisecond budgets above are defined **at 4× CPU throttling on a developer-class machine**, and are asserted by the **local** gate. On CI the same numbers are printed and attached to the run as annotations, but not asserted: a shared runner's wall clock measures the runner, and a ceiling nobody can meet is not a guard.
- The round trip is **store set → `ack`, end to end**, as written. The bridge-path number (the same payload posted and acked with no store write and no React render in between) is an additional diagnostic, not a replacement: it proves the bridge and the channel are inside budget, not that the phase's exit criterion is met.
...
| `in-view` firing condition | `isIntersecting && intersectionRatio >= T` | `isIntersecting && (intersectionRatio >= T \|\| reachable < T)`, where `reachable = min(1, rootW/w) * min(1, rootH/h)` — the largest ratio the element could ever attain, given how much of it fits in the root on each axis — and `rootW`/`rootH` come from `entry.rootBounds`, falling back to `window.innerWidth`/`innerHeight` when it is null (an exported page inside a cross-origin iframe; reading off null would throw and hold everything for ever). The second clause is DT-095: `intersectionRatio` is an **area** ratio, so an element bigger than the root can never reach 1 and one big enough can never reach `T` at all. Area, not height: a 4000px-wide track in a horizontal scroller reaches 0.16 with nothing wrong with its height, and a 1280×1200 element is short on neither axis alone. Still uncovered, and logged: a clip container narrower than the root. **The bridge adopts the same condition in plan task A12; until it does, this row is the one place preview and export differ by accident rather than by decision.** |
| `in-view` elements that arrive late | n/a: the shell re-sends state, and a clone has no scripts to add elements | a `MutationObserver` on `documentElement` (`childList`, `subtree`) observes marked elements added after `DOMContentLoaded`, so a snippet pasted into a client-rendered site behaves like one in a static page |
