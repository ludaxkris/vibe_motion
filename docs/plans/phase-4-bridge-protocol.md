...
| `in-view` | one shared `IntersectionObserver` for all in-view assignments | thresholds `[0, T]`, `T = IN_VIEW_THRESHOLD = 0.2`, the exported constant in `protocol.ts` that the Phase 7 exporter reuses. The `0` costs one more callback per entry and is what makes the reachability clause of §6a possible: an element that can never reach `T` has to be reported the moment it intersects at all. No per-frame work and no timer either way |
