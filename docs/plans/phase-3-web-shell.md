...
- `/dev` (index) replaces `/dev/panel` (redirect the old path): renders every panel state (idle empty, idle with assignments, selected, choosing, choosing with empty search, tuning for an entry with `distance` and one with `scale`), both dialogs (open, inline-rendered rather than portalled over each other), the toast, and the Entry cloning + error states, each in a labelled frame at its real width (panel 320px). Still 404 in production. Gallery frames must not mutate the shared `useEditorStore` at module load or leak sample drafts into it: give the per-state panel components explicit props (state, entry, assignment, callbacks) with the store-connected `ControlPanel` as a thin container, and render the gallery from props; remove `seedGalleryDraftOnce()`.
