...
| DT-047 | tech-debt | `keyframesName()` keys on catalog MAJOR only, so a MINOR can never add a cssVar param to an existing entry | P2 | this-phase | 1 | — | open | PR #3 added a validate gate: within one major, shared animationIds must have byte-identical `keyframes`/`baseStyles` (otherwise two pins collide on `@keyframes vm-<id>-v<major>`). Consequence: a new non-standard param on an existing entry needs a keyframes edit, i.e. a MAJOR. Options: keep (document it in README/CHANGELOG), or name keyframes by full version / content hash. Needs `code-architect` + Chris before Phase 4 runtime and Phase 7 exporter bake the name in. |
| DT-048 | tech-debt | Catalog/generated gates have two coverage gaps | P2 | this-phase | 1 | — | open | (a) `validate.mjs` same-major pass does not require `prior ids ⊆ later ids`, so a MINOR that drops an animation passes every gate. (b) `scripts/check-generated.mjs` silently skips a `generated[]` path that is missing both before and after regeneration (e.g. after a rename) and still prints OK. From PR #3 re-review (NEW-2, NEW-3). |
...
| DT-050 | feature | Resizable Control Panel (drag handle, clamp 20–30%) | P3 | later | 3 | — | open | Design handoff (`docs/design/`, PR #4) ships the editor with a fixed 320px panel and drops resize from v0; build_plan §4 Phase 3 says resizable, clamped 20–30%. Handoff wins on visuals: v0 is fixed-width. The handoff's own `deferred_tasks_additions.md` calls this DT-020, which collides with the existing DT-020; this is the real ID. |
